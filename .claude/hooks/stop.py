#!/usr/bin/env python3
# Claude Code Stop：全仓 lint 门控委托给 prek（lint 组，单一真相源）。本脚本只把
# prek 退出码翻译成 Claude 的 block 决定；stop_hook_active 守卫防循环（每轮至多
# 拦截一次）。绝不输出 continue: false（在部分平台语义为"停止"且优先级更高）。
...
def main() -> None:
    ...
    try:
        ...
        sys.stderr.write("[stop-hook] prek 不在 PATH 上，跳过 lint 门控\n")
        return
